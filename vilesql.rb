# typed: false
# frozen_string_literal: true

# This file was generated by GoReleaser. DO NOT EDIT.
class Vilesql < Formula
  desc "SQLite Database Management Tool with cloud hosting and secure authentication"
  homepage "https://github.com/imrany/vilesql"
  version "0.10.5"
  license "GPL-3.0-only"

  on_macos do
    url "https://github.com/imrany/vilesql/releases/download/v0.10.5/vilesql_0.10.5_Darwin_all.tar.gz"
    sha256 "36ef0526eff35facab15d2ac0c74cf4eb20176032bd1b1a51cd8bd09f6096940"

    def install
      bin.install "vilesql"
    end
  end

  on_linux do
    on_intel do
      if Hardware::CPU.is_64_bit?
        url "https://github.com/imrany/vilesql/releases/download/v0.10.5/vilesql_0.10.5_Linux_x86_64.tar.gz"
        sha256 "001261daccdd970cdf2841fa00cf555e8a7d902237b1cd1d34c8954959f9e409"

        def install
          bin.install "vilesql"
        end
      end
    end
    on_arm do
      if Hardware::CPU.is_64_bit?
        url "https://github.com/imrany/vilesql/releases/download/v0.10.5/vilesql_0.10.5_Linux_arm64.tar.gz"
        sha256 "6d8c66f967b8f89a8b222e6873a95078cf4e9099de2385ee6d8117c6d5b1abe4"

        def install
          bin.install "vilesql"
        end
      end
    end
  end

  def caveats
    <<~EOS
      VileSQL will create a data directory at ~/.vilesql on first run.
      This directory contains your database files and configuration.

      Data locations:
        - ~/.vilesql (user data)
        - ~/Library/Application Support/vilesql (macOS)

      To specify a custom data directory, use:
        vilesql --data-dir /path/to/your/data

      Upgrade VileSQL:
        brew upgrade vilesql
        # or use the built-in upgrade command:
        vilesql upgrade

      Check for updates:
        vilesql upgrade --check-only

      To completely uninstall VileSQL and remove all data:
        vilesql uninstall --remove-data
        brew uninstall vilesql

      Or backup before uninstalling:
        vilesql uninstall --remove-data --backup-data
        brew uninstall vilesql
    EOS
  end
end
